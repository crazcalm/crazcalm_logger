...
use crazcalm_logger::{LevelFilter, SimpleFormatter, StdoutLogger};

#[test]
fn test_stdout_logger() {
    let expected_log = r"File: tests\stdout_logger_test.rs, Level: INFO,";

    if env::var("YOU_ARE_TESTING_NOW").is_ok() {
        // Init from the env (which should set the max level to `Debug`)
        let _ = StdoutLogger::init(LevelFilter::Debug, SimpleFormatter::new()).unwrap();
    ...
    if out.status.success() {
        let result = str::from_utf8(&out.stdout).unwrap();
